...
from yearly_dashboard import YearlyDashboard

app = Flask(__name__)

# Favicon route with cache busting
...
yearly_dashboard = YearlyDashboard()
...
def index():
    ...
    <head>
        ...
        <!-- LifePlanner Rainbow Logo Favicons -->
        ...
        <!-- Apple Touch Icons -->
        <link rel="apple-touch-icon" sizes="180x180" href="/assets/branding/mobile_ready/ios/rainbow_logo_resized/icon-180x180.png">
        <link rel="apple-touch-icon" sizes="120x120" href="/assets/branding/mobile_ready/ios/rainbow_logo_resized/icon-120x120.png">
    ...
    <body>
        <div class="container">
            ...
            <div style="margin-top: 40px; padding-top: 30px; border-top: 2px solid #eee;">
                <h2 style="color: #333; font-size: 1.8em; margin-bottom: 20px;">📊 Quick Access</h2>
                <div style="display: flex; gap: 20px; justify-content: center; flex-wrap: wrap;">
                    <a href="/yearly-plan?kevin_type=working" style="background: #28a745; color: white; padding: 12px 25px; border-radius: 25px; text-decoration: none; font-weight: bold; transition: all 0.3s ease;">
                        📅 Working Kevin - Yearly Plan
                    </a>
                    <a href="/yearly-plan?kevin_type=job_searching" style="background: #007bff; color: white; padding: 12px 25px; border-radius: 25px; text-decoration: none; font-weight: bold; transition: all 0.3s ease;">
                        🚀 Job Search Kevin - Yearly Plan
                    </a>
                    <a href="/calendar/daily?kevin_type=working" style="background: #6c757d; color: white; padding: 12px 25px; border-radius: 25px; text-decoration: none; font-weight: bold; transition: all 0.3s ease;">
                        📱 Daily Calendar
                    </a>
                </div>
            </div>
        </div>
        <!-- Add Clickable Outcome Tracking -->
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script>
        <script src="/static/fixed_modal.js"></script>
        <script src="/static/clickable_outcome_tracking.js"></script>
    </body>
    </html>
    '''
...
def record_completion_api():
    ...
    # Return success response with demo progress data
    return jsonify({
        'success': True,
        'message': f'✅ Activity "{data.get("activity_id", "").replace("_", " ").title()}" recorded successfully!',
        'updated_info': {
            'current_streak': 19,  # Demo: increment streak
            'weekly_rating': 8.3,  # Demo: slight improvement
            'next_milestone': {
...
def unified_yearly_plan():
    """Unified yearly plan view for both Kevin versions"""
    kevin_type = request.args.get('kevin_type', 'working')
    return yearly_dashboard.generate_dashboard_html(kevin_type)

@app.route('/api/yearly-plan/<persona>')
def yearly_plan_api(persona):
    """API endpoint for yearly plan data"""
    if persona not in ['working', 'job_searching']:
        persona = 'working'
    # Simplified API response for now
    return jsonify({
        "persona": persona,
        "year": 2025,
        "message": "Yearly plan data available via HTML dashboard",
        "dashboard_url": f"/yearly-plan?kevin_type={persona}"
    })

@app.route('/working-kevin/yearly-plan')
def working_kevin_yearly_plan():
    """Yearly plan view for Working Kevin"""
    return yearly_dashboard.generate_dashboard_html("working")

@app.route('/job-search-kevin/yearly-plan')
def job_search_kevin_yearly_plan():
    """Yearly plan view for Job Search Kevin"""
    return yearly_dashboard.generate_dashboard_html("job_searching")
